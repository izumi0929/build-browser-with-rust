...
impl HtmlTokenizer {
    ...
    fn create_tag(&mut self, start_tag_token: bool) {
        if start_tag_token {
            self.latest_token = Some(HtmlToken::StartTag { tag: String::new(), self_closing: false, attributes: Vec::new() });
        } else {
            self.latest_token = Some(HtmlToken::EndTag { tag: String::new() });
        }
    }

    fn reconsume_imput(&mut self) -> char {
    ...
    loop {
        ...
        match self.state {
            ...
            State::ScriptDataEndTagOpen => {
                ...
                self.reconsume = true;
                self.state = State::ScriptData;
                // 仕様では</の２つの文字トークンを返すことになってるが、
                // この実装ではnextメソッドから１つのトークンしか返せないので、
                // ここでは`<`の文字トークンのみを返す
                return Some(HtmlToken::Char('<'));
            }
            ...
            State::TemporaryBuffer => {
                ...
                let c = self
                .buf.chars().nth(0).expect("self.buf should have at least 1 char");
                self.buf.remove(0);
                return Some(HtmlToken::Char(c));
            }
        }
